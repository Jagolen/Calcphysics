...
int main(int argc, char** argv){
    ...
    char filename_in[60] = "ellipse_N_03000.gal";
    ...
    double t, total_time, average_time;
    total_time = 0;
    for(t = 1; t < 5; t++){
        ...
        for(n = 0; n < nsteps; n++){
            for(i = 0; i < N; i++){
                ...
                //keep_within_box(&outputs[i*6 + 0], &outputs[i*6 + 1]);
                //ClearScreen();
                //DrawCircle(outputs[i*6 + 0], outputs[i*6 + 1], 1, 1, 0.025, 0);
            }
            for(k = 0; k < N; k++){
            ...
            //keep_within_box(&outputs[n*6 + 0], &outputs[n*6 + 1]); 
            //ClearScreen();
            //DrawCircle(outputs[n*6 + 0], outputs[n*6 + 1], 1, 1, 0.025, 0);
    ...
    //}
        ...
        double time_taken = get_wall_seconds() - start_time;
